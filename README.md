...
一个用于 vivo X100 Pro 的 Android MVP：从阿里商旅截图或分享文本中识别国航航班、日期、航段和舱位，自动查询国航凤凰知音官方累计计算器，返回该舱位对应的可用里程、定级里程和定级航段。
...
4. App 不需要读取账号，不需要悬浮窗，不需要无障碍权限；只处理你主动选择或分享过来的截图。
5. App 需要联网访问国航凤凰知音官方累计计算器接口。
...
- 支持手工修正字段后重新查询国航官方结果。
- 国航官方接口查询失败时，显示本地估算兜底，避免临时网络问题完全不可用。
- 支持复制结果、跳转国航官方累计计算器核对。

## 注意

- 官方查询结果来自国航凤凰知音累计计算器，国航最终入账仍以账户实际入账为准。
